...
Multi-context: `CONTEXT-MAP.md` at the root is the entry point. `IdentidadeEAcesso`, `CarteiraClientes` and `Credito` are materialised (`docs/contextos/<context>/CONTEXT.md`); remaining contexts stay in the root `CONTEXT.md`. ADRs live in `docs/adr/`. See `docs/agents/domain.md`.
