...
determina autoridade.** Uma proposta pode exigir decisão humana por causa do risco, e ainda assim
é preciso verificar se aquele aprovador tem alçada suficiente para o valor. A classificação de
risco não entra dentro do value object de alçada: se um dia surgir regra do tipo "casos
excepcionais de risco exigem nível superior", ela se expressa como política de roteamento e
aprovação, e não transformando `AlcadaAprovacao` num objeto que conhece toda a política de
crédito.

## Consequências

Uma decisão humana só é válida quando o aprovador possui alçada suficiente, o aprovador não é o
originador (ADR-0007), a proposta está em estado que permite decisão, e todas as análises
obrigatórias foram concluídas.
