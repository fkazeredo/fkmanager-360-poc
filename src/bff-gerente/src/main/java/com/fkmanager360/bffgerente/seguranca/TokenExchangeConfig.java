...
public class TokenExchangeConfig {
    ...
    OAuth2AuthorizedClientManager authorizedClientManager(
            ...
            @Value("${bff-gerente.carteira-clientes.audience}") String audienceCarteiraClientes) {
        ...
        tokenExchangeProvider.setSubjectTokenResolver(
                context -> resolverTokenDeLogin(context.getPrincipal(), authorizedClientRepository));
        ...
        tokenExchangeResponseClient.setParametersCustomizer(parametros -> parametros.add("audience", audienceCarteiraClientes));
        tokenExchangeProvider.setAccessTokenResponseClient(tokenExchangeResponseClient);

        OAuth2AuthorizedClientProvider cadeiaDeProviders = OAuth2AuthorizedClientProviderBuilder.builder()
                .authorizationCode()
                .refreshToken()
        ...
        manager.setAuthorizedClientProvider(cadeiaDeProviders);
        return manager;
    }

    private static OAuth2Token resolverTokenDeLogin(
        ...
        OAuth2AuthorizedClient clienteDeLogin = repository.loadAuthorizedClient(REGISTRATION_LOGIN, principal, request);
        return clienteDeLogin == null ? null : clienteDeLogin.getAccessToken();
    }
}
